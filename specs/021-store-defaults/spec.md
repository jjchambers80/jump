...
- **FR-007** `VenueService.createVenue` defaults `timezone` to the organization's `timezone` when the request omits it. **Spec 033 depends on this**: it lands a `defaultVenueTimeZone(organization)` helper in `VenueService` that returns the hardcoded `America/New_York` until this field exists — implementing FR-007 is a one-line change inside that helper, not a new code path. 033 also adds `Venue.country` and `Venue.timezoneSource`; a venue whose source is `DERIVED` resolves from its own postal code and must not be overwritten by the store zone. The store zone is the fallback for venues that never resolved, not a global override.
