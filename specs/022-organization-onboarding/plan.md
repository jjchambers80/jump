...
**Status**: Planned 2026-09-18. Phase 1 built 2026-09-18 (PR #77). Phase 2 built 2026-09-18 (`feat/022-onboarding-phase-2`): embedded Checkout + `confirm` on return (the return page and Settings › Plan both POST `confirm` so a slow webhook never leaves the organizer on the subscribe step), `BILLING_TRIAL_DAYS` instead of a `config/billing.js` constant, `NEXT_PUBLIC_BILLING_ENABLED` gates the nav entry; billing stays off without `JUMP_STARTER_PRICE_ID`. Phase 3 built 2026-09-18 (`feat/022-onboarding-phase-3`): templates seeded via `ApplicationFormTemplateService.create` (same validation as the editor); `eventTypes` → event category **dropped** (the create-event form has no category field); funnel counts come from `PlatformCustomer` + `onboardingCompletedAt`, abandoned signups are only in logs (deleted rows); the abandon threshold is `ONBOARDING_ABANDON_AFTER_MS`. Phase 1 deviations: the `PlatformCustomer` row is created at step 1 (with `source`) rather than lazily at survey/complete, so there is one write path; `Organization.onboardingCompletedAt` defaults to `now()` and only the signup flow writes `null`; there is no Navbar **Start selling** link because `Navbar.tsx` is not rendered anywhere — the sign-in page carries a **Create your organization** link instead.
