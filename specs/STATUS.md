...
| 022 | Organization onboarding (signup flow, Jump customer record, dashboard setup guide) | **Planned** | Plan written 2026-09-18 (`specs/022-organization-onboarding/plan.md`); no code. Phase 1: `/signup` name → survey → done, `PlatformCustomer`, creator membership + role promotion, switcher opens a new tab, dashboard setup guide. Phase 2: subscribe step + Settings › Plan behind `BILLING_ENABLED` (Stripe Billing, embedded Checkout). Phase 3: survey-tailored templates, abandoned-org sweep, funnel. Supersedes spec 005 US2 (wizard) and FR-009 (one org per account). |
