...
**Status**: Phase 1 implemented 2026-09-14 on branch `feat/009-tax-settings` (service card, regions table, edit dialog, backend + migration, tests, docs). Stripe Tax field names pinned against `stripe@17` types (§2.1). Phase 2 implemented 2026-09-14 on `feat/009-tax-phase-2` (Recalculate now, upcoming-event count, tax line on the admin event page). Phase 3 implemented 2026-09-14 on `feat/009-tax-phase-3` (collected tax report, tax-inclusive pricing with fees on the net amount per §5.2 recommendation). §5.1 (tax on service fees) and §5.4 (seller of record) remain product decisions; §5.3 shipped as recommended.
