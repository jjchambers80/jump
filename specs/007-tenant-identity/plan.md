...
- Host resolution: `frontend/src/middleware.ts` calls `GET /domains/resolve` (60s cache on both sides). Middleware had to move into `src/` — the root `frontend/middleware.ts` was never loaded by Next in this layout, so the Auth.js protection it contained never ran. Staff protection was restored on the edge in a follow-up (`fix/edge-admin-protection`): the HS256 cookie codec moved to `auth.config.ts` (`lib/authJwt.ts`, `jose`), and the middleware's Auth.js instance omits the email provider (needs an adapter; caused `MissingAdapter` on the edge). `/admin*` without a session now redirects to sign-in before any page code runs.
