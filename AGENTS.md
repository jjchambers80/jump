...
17. Transactions (spec 018): `/admin/transactions` is a raw-SQL `UNION ALL` over orders and applications (`backend/src/services/transactionQuery.js`) — enum columns compare as `::text`, dates as `($n::timestamptz AT TIME ZONE 'UTC')`, every value a `$n` parameter. Order refund routes require ADMIN (same as application refunds). See `backend/AGENTS.md`
