...
29. Disputes (spec 037): `charge.dispute.*` all land on `DisputeService.applyFromEvent` through the existing signature-verified `POST /webhooks/stripe`. Handlers derive **full state from the dispute object** — `fundsWithdrawn` from `dispute.status` plus the sign of Σ `dispute.balance_transactions`, never from the event type — and `Dispute.lastEventAt` is monotonic, so a strictly older event is ignored. That pair is what makes a redelivery a no-op and stops a late `.funds_withdrawn` from taking money back out of a dispute Jump won; never branch dispute behaviour on the event type. There is deliberately **no `DISPUTED` OrderStatus**: the withdrawn money is a `Refund` row (`disputeId` unique, `stripeRefundId` null), so `refunded` / `net`, analytics and the tax report stay correct and the existing `REFUNDED` / `PARTIALLY_REFUNDED` pair already says money left — what tells a chargeback apart is the `Dispute` row on the order and `Refund.disputeId`. A won dispute marks that row `FAILED` and restores exactly the tickets and add-on lines this dispute closed, with their prior status. `npm run report:disputes` counts both directions. See `docs/wiki/features/disputes-chargebacks.md`
