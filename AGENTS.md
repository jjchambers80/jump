...
29. Stripe webhooks (EVE-3): the three endpoints **fail closed** — with the signing secret unset, `NODE_ENV=production` **or a live `sk_live_` key** answers 500 (not 400, so Stripe's three-day retry drains the backlog once the secret lands) instead of trusting the body. Every delivery is recorded in `StripeWebhookEvent`, unique on `(endpoint, stripeEventId)`; that insert is the dedup lock, so a redelivery answers `{received, duplicate: true}` without reaching a handler, while a previously `FAILED` event is deliberately let back through. The ledger **fails open** — if the row cannot be written the event still processes. **A contract test that posts a webhook must namespace its Stripe event id per run and clear the rows in `afterAll`**, or its second run against the same database is a replay that never reaches a handler. `createStripeRefund` **requires** an `idempotencyKey` and throws without one; keys are derived from the operation (`ticket:<id>`, `order:<id>:full`, `order-add-on:<id>`), never from a `Refund` row id a rolled-back transaction would discard. `npm run verify:stripe` / `npm run verify:checkout-tax` exercise the real thing against Stripe test mode and are deliberately outside `npm test`. See `docs/wiki/features/webhook-reliability.md` and the founder runbook `docs/wiki/config/stripe-live-activation.md`
