...
29. Cancelling an event notifies and voids, but **does not refund**. `EventService.cancelEvent` emails every live ticket holder (or RSVP guest) and flips every `VALID` ticket to `VOIDED` in the cancelling transaction; `REDEEMED` tickets are notified but keep their attendance record, and tier inventory is deliberately not restored. Idempotency comes from the terminal-status guard — a second cancel is a 409. No money moves: who owes a cancellation refund is an open policy question, so `EmailService.sendCancellationNotification` must not promise one until it does. At the door, voiding covers the tickets that existed; `TicketService._assertEventNotCancelled` covers a ticket minted afterwards by a late webhook (`EVENT_CANCELLED` on scan, redeem and admin check-in). See `docs/wiki/features/event-management.md`.
