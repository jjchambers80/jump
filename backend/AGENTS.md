...
1. Frontend Auth.js handles login (Google OAuth, magic link, `password` and `token-bridge` Credentials providers — see Account settings below)
...
- Sign-in methods (feature B): mutations under `/account/*` that change how a user signs in take `requireRecentAuth` (`X-Jump-Reauth` proof from `POST /account/reauth`, 10 min) — add it to any new one. Passwords are scrypt via `utils/password.js`; one-time codes/links go through `utils/oneTimeTokens.js` (never store a raw token). `POST /auth/password` is frontend-only (`X-Jump-Internal`). Ceremonies that end outside Auth.js (passkey, recovery) mint a `bridge:` token that only the frontend `token-bridge` provider consumes.
- Two-step (feature C): a staff JWT may carry `mfa: 'pending'`; `requireAuth` refuses it (401 `TWO_STEP_REQUIRED`) and only `routes/twoStep.js` (mounted at `/account/two-step` **before** `/account`) uses `requireAuthAllowPending`. TOTP seeds are sealed with `utils/secretBox.js` (key from `AUTH_SECRET`); recovery codes and trusted-device tokens are sha256 hashed. `verify` returns a single-use proof (`mfa-proof:<jti>`) that only the frontend `update({ mfaProof })` path redeems.
...
- `createOrder` refuses (409) once one email holds `ORDER_MAX_PENDING_PER_CONTACT` PENDING orders on an event — checked inside the transaction before any tier is reserved. `OrderService.sweepAbandoned()` (timer in `server.js`) fails PENDING orders whose Checkout session expired without the webhook and completes paid ones the webhook missed. See `docs/wiki/features/abuse-protection.md`.
...
src/services/         # Business logic: domain services + small helpers like applicationLinks.js, stripeRefund.js
