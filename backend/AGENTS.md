...
4. A Stripe-id search (`pi_`, `cs_`, `re_`, `pyr_`, `ch_`) skips `ILIKE` and compares id columns by equality; `ch_` is resolved to its PaymentIntent with one `charges.retrieve`. See `docs/wiki/features/transactions.md`.
