...
//
// PARITY: this file and `backend/src/utils/usTimeZones.js` must behave
 ...
 */
const SPLIT_STATES = new Set<string>([
 ...
 */
const ZIP_PREFIX_OVERRIDES: Record<number, string> = {
 ...
 */
const SPANNING_PREFIXES = new Set<number>([
...
function zipPrefix(value: unknown): number | null {
  const digits = typeof value === 'string' ? value.replace(/\D/g, '') : '';
  return digits.length >= 3 ? Number(digits.slice(0, 3)) : null;
}

export type VenueTimeZoneReason =
  | 'ZIP'
  | 'STATE'
  | 'SPLIT_STATE'
  | 'ZIP_SPANS_BOUNDARY'
...
export interface VenueTimeZoneResult {
  /** null when nothing can be said — the caller falls back and stays DEFAULT. */
  timezone: string | null;
  /** false means "this is a guess, ask the organizer to confirm". */
  confident: boolean;
  reason: VenueTimeZoneReason;
}
...
/** Resolve a venue's time zone from its address. */
export function resolveVenueTimeZone({ country, state, postalCode }: VenueAddressParts = {}): VenueTimeZoneResult {
