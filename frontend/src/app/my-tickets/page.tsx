      ...
      <div className="flex items-center justify-between">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <h4
              className={`text-sm font-medium truncate ${isExpired ? 'text-gray-500' : 'text-gray-900'}`}
            >
        ...
        <div className="text-right ml-4">
          <p className={`text-sm font-semibold ${isExpired ? 'text-gray-400' : 'text-gray-900'}`}>
            {formatPrice(ticket.pricePaid)}
          </p>
          <svg
            className={`w-4 h-4 ml-auto mt-1 ${isExpired ? 'text-gray-300' : 'text-gray-400'}`}
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
...
function EventGroup({ group }: { group: GroupedTickets }) {
  ...
  return (
    <div
      className={`rounded-xl border ${past ? 'border-gray-200 bg-gray-50/50' : 'border-gray-200 bg-white'} overflow-hidden`}
    >
      {/* Event Header */}
      <div
        className={`px-5 py-4 border-b ${past ? 'border-gray-200 bg-gray-100/50' : 'border-gray-100 bg-gradient-to-r from-indigo-50 to-white'}`}
      >
        <h3 className={`text-lg font-bold ${past ? 'text-gray-500' : 'text-gray-900'}`}>
          {group.eventName}
        </h3>
        <div className="flex flex-wrap gap-4 mt-1 text-sm">
          <span className={`flex items-center gap-1 ${past ? 'text-gray-400' : 'text-gray-600'}`}>
          ...
          <span
            className={`text-xs px-2 py-0.5 rounded-full ${past ? 'bg-gray-200 text-gray-500' : 'bg-indigo-100 text-indigo-700'}`}
          >
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-3 text-gray-500">Loading your tickets...</p>
        </div>
      </div>
    );
  }
