...
// Full-bleed dark stage for the signup steps (reference:
  ...
  return (
    ...
    >
      ...
      <div className="max-w-xl mx-auto mt-6 sm:mt-10">
        ...
        {subtitle && <p className="mt-2 text-center text-sm sm:text-base text-white/60">{subtitle}</p>}
        ...
        <div className="relative mt-8">
          {onBack && (
            <button
              ...
              disabled={busy}
              aria-label="Back"
              className="absolute -left-14 top-2 hidden sm:flex items-center justify-center w-11 h-11 rounded-full bg-white/10 hover:bg-white/20 transition disabled:opacity-50"
            >
          ...
          <div className="rounded-2xl bg-white text-gray-900 p-5 sm:p-6 shadow-2xl">
            {onBack && (
              <button
                type="button"
                onClick={onBack}
                disabled={busy}
                className="sm:hidden mb-3 text-sm text-gray-500 hover:text-gray-900"
              >
                ← Back
              </button>
            )}
