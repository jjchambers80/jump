...
Add-ons (spec 012): AddOn per event (scope TICKET/APPLICATION/BOTH, `allTiers`) → PriceTierAddOn / ApplicationTierAddOn attachments; OrderAddOn lines on Order (immutable unitPrice + allocated fees/tax, `refundedAt`); ApplicationAddOn lines on Application (`unitPrice` + `applicantPays` share of the snapshot); Refund.orderAddOnId
...
Online Store › Preferences: `Organization.storefrontPrivate` / `storefrontPasswordHash` / `storefrontMessage` / `seoTitle` / `seoDescription` / `autoRedirectLanguage` (`20260929000000_online_store_preferences`). The client in `src/index.{js,ts}` is built with `omit: { organization: { storefrontPasswordHash: true } }`, so the hash is absent from every query result unless it passes `omit: { storefrontPasswordHash: false }` or selects it explicitly. See `docs/wiki/features/online-store-preferences.md`.
...
Add-ons (spec 012): `AddOn.quantityTotal` null = unlimited; `quantitySold` / `quantityReserved` move only through `AddOnService.reserve/commit/release/unsell` (conditional raw UPDATE). `OrderAddOn` / `ApplicationAddOn` are unique on `(orderId|applicationId, addOnId)`; add-on lines are never `OrderItem`s. Migrations `20260918000000_add_ons`, `20260919000000_add_ons_applications` (`ApplicationAction.ADD_ONS_CHANGED`, `ApplicationAddOn.applicantPays`).
