...
| `STRIPE_WEBHOOK_SECRET` | Yes | Stripe webhook signing secret (`whsec_...`). Get from `stripe listen` CLI output locally. **Unset = every event on `POST /webhooks/stripe` is refused with 500**, unless `STRIPE_WEBHOOK_ALLOW_UNSIGNED=true` |
| `STRIPE_WEBHOOK_ALLOW_UNSIGNED` | No | **Local development only.** `true` accepts an unsigned webhook body when no signing secret is set, so `stripe trigger` works without `stripe listen`. Any other value (including unset, `1`, `yes`, `TRUE`) refuses. Never set it on a deployed service: an unverified webhook endpoint is an unauthenticated write path into the ledger |
...
| `STRIPE_CONNECT_WEBHOOK_SECRET` | With Connect | Signing secret for the *Connect* webhook endpoint (`POST /webhooks/stripe/connect`, "listen to events on connected accounts"). Different from `STRIPE_WEBHOOK_SECRET`. Unset = every Connect event is refused with 500 unless `STRIPE_WEBHOOK_ALLOW_UNSIGNED=true` |
...
| `STRIPE_BILLING_WEBHOOK_SECRET` | With billing | Signing secret for `POST /webhooks/stripe/billing` (subscription events). Same account as `STRIPE_WEBHOOK_SECRET`, separate endpoint. Unset = every billing event is refused with 500 unless `STRIPE_WEBHOOK_ALLOW_UNSIGNED=true` |
