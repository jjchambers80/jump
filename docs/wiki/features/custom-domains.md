...
- **Middleware location.** This project uses the `src/` layout, so Next loads `frontend/src/middleware.ts` only. The old `frontend/middleware.ts` at the package root was never executed — including the Auth.js route protection it contained. Staff admin routes are protected client-side (`AdminRoute`/`ProtectedRoute`); the middleware deliberately does not call Auth.js because `auth.config.ts` lacks the custom HS256 `jwt.decode` and would reject every session cookie on the edge.
