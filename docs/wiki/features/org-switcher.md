...
2. `resolveOrgScope(userId, role, preferredOrgId)`:
   - `SYSTEM_ADMIN` → `{ organizationId: null, venueFilter: undefined }` (unscoped).
...
- **`SYSTEM_ADMIN` has no memberships.** `resolveOrgScope` returns unscoped; any route that needs a concrete org must use `activeOrgFor(req)` (or the same fallback chain). Resolving from "first membership" yields `null` and a 404 for every `SYSTEM_ADMIN` (PR #28).
