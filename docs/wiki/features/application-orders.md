...
| `packages/db/prisma/migrations/20260930100000_legal_acceptance_and_opt_ins` | Phase 3: `LegalAcceptance` (+ `LegalSubject` / `LegalDocument` / `LegalSource`), `Contact.emailSubscribedAt / emailSubscribedSource / emailUnsubscribedAt` (+ `EmailSubscribedSource`), `Application.optInAccount / optInMarketing / optInsAppliedAt` |
| `backend/src/config/legal.js` | Phase 3: `LEGAL_VERSIONS` (all `-draft` until spec 023 ships text), `checkoutAcceptanceRequired()` (`LEGAL_ACCEPTANCE_REQUIRED`), `cardAuthorizationText`, `applyConsentText` |
| `backend/src/services/LegalAcceptanceService.js`, `api/routes/legal.js` | Phase 3: `assertCurrent(acceptances, required)` → 400 `LEGAL_ACCEPTANCE_REQUIRED` / `LEGAL_VERSION_STALE` (error `code` surfaced by the error handler), `record(tx, …)` (append-only, `ipHash` = sha256(ip + day + salt), UA ≤ 255, `presentedText`), `requestMeta(req)`; `GET /legal/versions` (public) |
| `backend/src/services/ContactOptInService.js` | Phase 3: one implementation of "create an account" / "email me" for checkout (`PaymentService._applyOptIns`, source `CHECKOUT`) and applications (`applyForApplication`, source `APPLY`, once per application via `optInsAppliedAt`); `welcomeUrl` (WELCOME token); `marketingChangeData` for staff edits (source `ADMIN`, `emailUnsubscribedAt`) |
| `backend/src/services/ApplicationService.js` (`submit`), `ApplicationPaymentService.js` (`_sendReceived`) | Phase 3: opt-ins stored on the application; acceptances required (TERMS, PRIVACY, and CARD_AUTHORIZATION on PAID + APPROVAL forms) and recorded in the submission transaction; FREE forms apply opt-ins on creation, PAID forms when the card step makes them SUBMITTED; the RECEIVED email carries the first sign-in link (`account.created` / `links.account`) |
| `backend/src/services/OrderService.js` (`createOrder`), `api/routes/orders.js` | Phase 3: `acceptances` validated and recorded (source `CHECKOUT`, reference `Order`); missing → logged unless `LEGAL_ACCEPTANCE_REQUIRED`; stale → always 400 |
| `frontend/src/lib/legal.ts`, `events/[eventId]/apply/[formSlug]/page.tsx`, `checkout/[eventId]/page.tsx` | Phase 3: `fetchLegalVersions` (cached per load), `acceptancesFor`, mirrored texts; apply form checkboxes (account default on, marketing, required consent, required card authorization naming the amount and the pay-now window), `LEGAL_VERSION_STALE` refetch; checkout echoes the versions and links the sentence only when `NEXT_PUBLIC_LEGAL_PAGES_ENABLED` |
...
## Apply-form account, subscription and consent (phase 3)

The apply form's "Your details" section offers **Create an account with {organizer}** (default on; magic-link sign-in, no password), **Email me about future events** (default off), and a required **I agree to {organizer} and Jump collecting and storing the information in this application** (the Privacy Policy link appears once `NEXT_PUBLIC_LEGAL_PAGES_ENABLED` is on). A PAID form that charges at approval adds a required authorization naming the amount, the trigger ("only if my application is approved") and the pay-now window (`paymentDueDays`). The client fetches `GET /legal/versions` and echoes them as `acceptances`; the backend refuses a stale version (400 `LEGAL_VERSION_STALE`, the form refetches) or a missing required document (400 `LEGAL_ACCEPTANCE_REQUIRED`) before anything is written, then records one `LegalAcceptance` per required document inside the submission transaction (subject the contact, source `APPLY`, reference the application, hashed IP, user agent, the server-rendered `presentedText` for the privacy consent and the card authorization).

Opt-ins are stored on the application (`optInAccount`, `optInMarketing`) and applied to the contact **once it first reaches SUBMITTED** (`ContactOptInService.applyForApplication`, stamped in `optInsAppliedAt`): on creation for FREE forms, after the card step for PAID forms. An abandoned DRAFT never creates an account or a subscription. Both only ever turn on; `emailSubscribedSource = APPLY` and `emailSubscribedAt` record the provenance. A just-created account gets a one-time WELCOME sign-in link in the RECEIVED email (`{{#account.created}}…{{links.account}}{{/account.created}}` in the default template).

Checkout keeps its sentence under the pay button (no checkbox, plan decision 7.9) and sends the versions it showed; `OrderService.createOrder` records `TERMS` + `PRIVACY` on the order (source `CHECKOUT`). A client that sends none is logged (`legal_acceptance_missing`) until `LEGAL_ACCEPTANCE_REQUIRED=true` — the switch to flip with the legal pages — after which it is refused; a stale version is always refused. Staff edits of a customer's marketing flag record source `ADMIN` and `emailUnsubscribedAt`.
...
- `backend/tests/contract/applyConsent.test.js` (phase 3) — `GET /legal/versions`; FREE form opt-ins applied with provenance, WELCOME token and sign-in link in RECEIVED, consent rows with hashed IP / UA / presented text; existing account untouched; stale / missing / unknown acceptances refused with nothing written; PAID + APPROVAL requires the card authorization, opt-ins wait for the card step, an abandoned DRAFT never applies them, the webhook replay applies nothing twice; checkout records on the order, logs when missing, refuses when `LEGAL_ACCEPTANCE_REQUIRED`, refuses stale; staff marketing edits carry ADMIN provenance. `tests/unit/legalAcceptance.test.js` — version checks, IP hash, texts. `tests/helpers/legal.js` — `acceptances()` / `allAcceptances()` for every submission test.
- `frontend/e2e/applications.spec.ts`, `applications-add-ons.spec.ts`, `add-ons.spec.ts` (phase 3) — the checkboxes, consent and card-authorization gating before anything is sent, the `acceptances` payload on apply and checkout, no links while the legal pages are dark (`e2e/helpers/legal.ts` mocks `GET /legal/versions`).
...
- Spec 023 phase 1 publishes the legal text: bump `LEGAL_VERSIONS`, set `NEXT_PUBLIC_LEGAL_PAGES_ENABLED` and `LEGAL_ACCEPTANCE_REQUIRED`, replace the engineering copy in `config/legal.js` / `lib/legal.ts` with counsel's wording (the two must stay in step; the backend rendering is what is stored).
