...
5. **Frontend cookie.** The verify page POSTs to `/api/buyer/verify`, which calls the backend and sets `jump_buyer` (httpOnly, SameSite=Lax, Secure in production, 30 days). `/api/buyer/me`, `/me/orders`, `/me/tickets` read the cookie server-side and forward it as a bearer token. `GET /api/buyer/me` answers `200 null` when signed out (no cookie, or the backend rejects it and the cookie is cleared) instead of a 401, so guest page views log no console error; callers treat `null` as signed out. The browser never holds the JWT, and the cookie is first-party, which is what lets this work unchanged on custom domains later.
