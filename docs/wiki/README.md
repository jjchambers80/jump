...
- [Customer Accounts Settings](features/customer-accounts-settings.md) — Settings › Customer accounts (spec 031): sign-in links toggle (storefront header + checkout, signed-in prefill, `?next=` return), account URL card; refund policy + code sign-in planned
