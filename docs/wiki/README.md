...
- [Vendor door check-in](features/vendor-door-check-in.md) — Event-day arrivals on a phone at the loading dock: roster search, scan of the status-link pass the vendor already has, `arrived / expected` with booth assignments, undo. The arrival stays `Application.checkedInAt`; the stamp is a conditional update on `checkedInAt IS NULL`, so a double-tap, a retried request and two staff scanning at once all produce exactly one arrival, which is what lets the client retry blindly on venue wifi (spec 036)
- [Public vendor directory](features/vendor-directory.md) — Approved, organizer-opted-in PAID-form vendors on the public event map: safe profile fields, photo fallback, category/search filtering, booth location, stable event-scoped ordering, and private-field exclusion (spec 014 phase 3)
...
- [Live Stripe Activation Runbook](config/live-stripe-activation-runbook.md) — The ordered go-live sequence: the three gating decisions, live keys, descriptor prefix, the three webhook endpoints, Connect, Tax, and a live end-to-end payment test, with rollback per phase
