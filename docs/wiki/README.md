...
- [Transactions](features/transactions.md) — Org-wide list of ticket orders and application payments from one `UNION ALL` read model (no new table): search by email / name / business / `orderRef` / Stripe id, type / status / event / date / has-refunds filters, streamed CSV with refund rows, refunds for either type delegating to the per-type services, order refund routes now ADMIN; customers, event analytics, dashboard and the collected-tax report include application money; pre-payment corrections on applications — tier change, signed adjustments folded into the tier line, ADMIN waive / offline payment (`paymentSource: OFFLINE`), recorded refunds (spec 018)
