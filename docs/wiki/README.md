...
- [Organization Storefront Layout](features/organization-storefront-layout.md) — `/organizations/[orgId]`: header → bounded full-bleed cover band → `max-w-7xl` events grid (1 / 2 at `sm` / 3 at `xl`); equal-height `EventCard`s, loading skeleton, empty state, tap-target floors
